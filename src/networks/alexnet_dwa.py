...
class Net(torch.nn.Module):
    ...
    Args:
        ...
        use_combination (bool): Defines if the attention mask should be computed through a combination of layers to save weights (applies only to dense layers)
    '''

    def __init__(self,inputsize,taskcla, use_processor=True, processor_feats=(32, 128), emb_size=100, use_stem=1, use_concat=False, use_combination=True, use_dropout=False, gate="sigmoid"):
    ...
    def _create_mask(self, out_size, use_combination=False):
        # compute the actual size
        self.mask_shapes.append(out_size)
        flat_shape = np.prod(out_size)

        # generate the layers
        if use_combination is True:
    ...
    def _create_linear(self, fin, fout, pos, stem, psize):
        ...
        if stem is not None and pos <= stem:
            ...
            self._create_mask((fout, fin), self.use_combination)
